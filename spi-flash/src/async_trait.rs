...
pub trait SpiNandAsync<SPI: SpiDevice>: SpiNand {
    ...
    async fn reset(&self, spi: &mut SPI) -> Result<(), SpiFlashError<SPI>> {
    ...
    async fn read_jedec_id(&self, spi: &mut SPI) -> Result<JedecID, SpiFlashError<SPI>> {
        let mut buf = [0; 2];
    ...
    async fn read_status_register_1(&self, spi: &mut SPI) -> Result<u8, SpiFlashError<SPI>> {
    ...
    async fn read_status_register_2(&self, spi: &mut SPI) -> Result<u8, SpiFlashError<SPI>> {
    ...
    async fn read_status_register_3(&self, spi: &mut SPI) -> Result<u8, SpiFlashError<SPI>> {
    ...
    async fn is_busy(&self, spi: &mut SPI) -> Result<bool, SpiFlashError<SPI>> {
    ...
    async fn check_ecc(&self, spi: &mut SPI) -> Result<ECCStatus, SpiFlashError<SPI>> {
    ...
    async fn write_enable(&self, spi: &mut SPI) -> Result<(), SpiFlashError<SPI>> {
        spi_write(spi, &[Self::WRITE_ENABLE_COMMAND]).await
    }

    /// Disable writing to the flash device
    async fn write_disable(&self, spi: &mut SPI) -> Result<(), SpiFlashError<SPI>> {
        spi_write(spi, &[Self::WRITE_DISABLE_COMMAND]).await
    }

    /// Check if write protection is enabled
    async fn is_write_enabled(&self, spi: &mut SPI) -> Result<bool, SpiFlashError<SPI>> {
        Ok((self.read_status_register_3(spi).await? & 0x02) != 0)
    }

    /// Check if programming/writing failed
    async fn program_failed(&self, spi: &mut SPI) -> Result<bool, SpiFlashError<SPI>> {
        Ok((self.read_status_register_3(spi).await? & 0x08) != 0)
    }

    /// Check if erase failed
    async fn erase_failed(&self, spi: &mut SPI) -> Result<bool, SpiFlashError<SPI>> {
